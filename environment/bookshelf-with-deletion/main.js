...
const deleteBook = (bookId) => {
  // TODO: books から該当の書籍データを削除　質問 7/13のコードを参考にする
    firebase
    .database()
    .ref("books")
    .child(bookId)
...
const createBookDiv = (bookId, bookData) => {
  ...
  // 削除ボタンのイベントハンドラを登録　質問　下記3行を追加してみたが違う
...
const loadBookshelfView = () => {
  ...
  booksRef.on('child_removed', (bookSnapshot) => {
    ...
    console.log("firebaseのデータが削除されているか確認");　

    // TODO: 書籍一覧画面から該当の書籍データを削除する 画面上のオブジェクトを取得してレッスン4
    // 削除しようとしているは該当の書籍。該当の書籍bookIdと、 $(`#book-id-${bookId}`)が≒でないといけない
    // if (bookId === $book) {
    $book.remove();
    // }
  });
...
// 書籍の登録モーダルを初期状態に戻す
// resetはJava Scriptの関数なのでjavascript で扱えるノードが必要だが$('#book-form') は jQuery のオブジェクトを返してしまうため
// このままだと使えない
// ノードは、HTML上のどの位置なのかを示すものであり、
// [‘#book-form’]から
// 配列から一番目の要素を取り出すことで、
// javascript で扱えるノードを取り出すことができる
