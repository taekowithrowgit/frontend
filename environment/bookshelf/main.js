...
// 書籍の表紙画像を表示する
//本の表示をする対象のHTML要素が入った変数と、画像のURL自体が displayBookImageに入っている
// attrというのは、対象の要素の属性値を編集したりすることができる　src属性の値にurl変数の中身をセットしている。
// imgタグにて、src属性で表示する画像を示している
...
const createBookDiv = (bookId, bookData) => {
  ...
  // id属性をセット
  //本ごとに、HTML要素を作成し、それに対して、HTMLにID属性として、追加している。
  // attr属性はもとからあるクラスやidを上書きすることができる
...
// 書籍の登録モーダルを初期状態に戻す
// $('#book-form')[0].reset();の部分だが、resetはJQueryではなく、JavaScriptの関数であり、JSで使用できるノードが必要
// ノードはHTML上のどの位置かを示すものであり、'#book-form'から一番目の要素を取り出すことで、JSで扱えるノードを取り出すことができる
